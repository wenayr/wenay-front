import {
    ...
    IParamExpandable,
    IParamsExpandable,
    IParamsExpandableReadonly,
    UserNumRange,
    UserTimeRange
} from "wenay-common/lib/Exchange/CParams";
...
import {
    ...
    isDate,
    TF,
    timeLocalToStr_yyyymmdd,
    timeLocalToStr_yyyymmdd_hhmm,
    timeLocalToStr_yyyymmdd_hhmmss,
    timeLocalToStr_yyyymmdd_hhmmss_ms
} from "wenay-common";
...
function InputTime(set: (data: string) => void, value: string | const_Date, range?: UserTimeRange) { //, type? :number|"time") { //}, validRange? :Partial<NumRange>){
...
const _inputNumStrMap = new WeakMap<Readonly<UserNumRange>, { min: string, max: string, step: string, val: string, value: number }>();
function InputNumber(set: (data: number)=>void, value: number, range: Readonly<UserNumRange>) { //}, validRange? :Partial<NumRange>){
    //if (1) return null;
...
export function ParametersReact<TParams extends IParamsExpandableReadonly = IParamsExpandableReadonly>(data : {
    params: TParams,
...
function ParametersBaseReact<TParams extends IParamsExpandableReadonly = IParamsExpandableReadonly>(data : {
    params: TParams,
    expandStatus?: boolean,
    expandStatusLvl?: number,
    onChange: (params: TParams) => void,  // при изменении значения
    ...
    function Param( //<T extends boolean|number|string|const_Date>(
        ...
        range?: Readonly< UserNumRange | UserTimeRange | number[] | string[] | const_Date[] >,
        labels?: readonly string[])
    {
        if (typeof(val)=="boolean") return CheckBox(set, val);
        if (Array.isArray(range)) return InputList(set, val, range, labels);
        if (typeof(val)=="number" && range) return InputNumber(set, val, range as UserNumRange);
        if ((typeof(val)=="string" && type=="time") || val instanceof Date) return InputTime(set, val, range as UserTimeRange);
        if (typeof(val)=="string") return InputString(set, val);
        return null;
    }

    function onSetValueA(param: IParam, value: unknown, enabled: boolean | undefined) {
        Refresh();
        if (enabled)
    ...
    function onExpandA(param: IParam, flag: boolean) {
        data.onExpand?.(myParams.current as TParams);
    }

    function ListParams(obj: IParamsExpandable, parentEnabled: boolean | undefined = true, expandLevel: number = 0) {
        return Object.entries(obj).map(([key, param]: [string, IParamExpandable]) => {
            ...
            if (typeof(param)=="object") {
                ...
                if (isDate(value)) {
                    ...
                    if (Array.isArray(value)) {
                        ...
                        if (Array.isArray(range)) {  // значит рисуем выпадающий список
                            ...
                            return simpleParameter(InputListArray(set, arr3, timeToStrings(range), (param as IParamEnum).labels));
                        }
                        if (! type) {
                        ...
                        const elements = arr.map((itemVal, index: number, array) => {
                                ...
                                const paramL = (hover: boolean) =>
                                    <CParameter name={<>{hover && !param.constLength && onHoverElement}{nameElement}</>}>
                                        {
                                            Param(onSet, itemVal, type, range, (param as IParamEnum).labels)
                                        }
                ...
                {
                    return simpleParameter(Param(set, value, undefined, range, (param as IParamEnum).labels));
                }
            }
            return null;
        })
